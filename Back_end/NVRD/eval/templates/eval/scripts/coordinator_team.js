function team_refresh_coordinator() {
    ...
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            svgstr += '</svg>'
            for (let i in data) {
                // str += '<tr><td scope="row" style="text-align:right"><input name="team_boxes_coordinator" class="form-check-input position-static" type="checkbox"></input></td>'
                str += '<tr>'
                for (let j in data[i]) {
                    str += ("<td>" + data[i][j] + "</td>")
                }
                // str += '<td scope="col"><button type="button" class="btn btn-dark active btn" data-toggle="modal" data-target="#modal_coordinator_team_put" onclick="coordinator_team_put_form(this)">' + svgstr + '</button></td></tr>'
                str += '</tr>'
            }
            document.getElementById("team_body_coordinator").innerHTML = str
        }
    }
    ...
    refreshLoader(xhttp)
    xhttp.open("GET", "/api/" + getCookie("username") + "/" + panel_id + "-" + panel_year_code + "/team/", true);
    xhttp.setRequestHeader("Authorization", "Bearer " + getCookie("token"));
    xhttp.setRequestHeader("X-CSRFToken", getCookie("csrftoken"));
    xhttp.send();
}

function team_search_coordinator() {
    tbody = document.getElementById("team_body")
    var xhttp = new XMLHttpRequest()
    xhttp.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
    ...
    refreshLoader(xhttp)
    var team_year_code = document.getElementById("team_search_team_year_code_coordinator").value
    var team_id = document.getElementById("team_search_team_id_coordinator").value
    var team_name = document.getElementById("team_search_team_name_coordinator").value
